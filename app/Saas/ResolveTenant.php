...
final class ResolveTenant
{
    public function handle(Request $request, Closure $next)
    {
        ...
        if (config('saas.dedicated_tenant_id') && ! in_array(strtolower($request->getHost()), config('saas.dedicated_hosts', []), true)) {
            abort(404);
        }
        ...
        try {
            $tenant = app(TenantRegistry::class)->byHost($request->getHost());
        } catch (\Illuminate\Database\QueryException $e) {
            return response()->json(['code' => 'PLATFORM_UNAVAILABLE', 'message' => 'Autorização temporariamente indisponível.'], 503);
        }
        if (! $tenant) {
            return response()->json(['code' => 'TENANT_NOT_FOUND', 'message' => 'Empresa não encontrada.'], 404);
        }
        if ($tenant->status !== 'active') {
            return response()->json(['code' => 'TENANT_UNAVAILABLE', 'message' => 'Ambiente indisponível. Contate a administração.'], 403);
        }
